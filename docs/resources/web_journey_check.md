...
- `actions` (Block Set) The set of actions to perform at the end of the step such as clicking on elements or enterting text. (see [below for nested schema](#nestedblock--step--actions))
- `common_step_id` (Number) If type is set to COMMON, then this should be set. The id of the Common Web Journey Step to use.
- `console_message_suppression` (Block Set) Suppress one or more cosole log messages from creating a warning or failure for a Web Journey Step. (see [below for nested schema](#nestedblock--step--console_message_suppression))
- `network_suppression` (Block Set) Suppress one or more network calls from causing any warnings or failures. (see [below for nested schema](#nestedblock--step--network_suppression))
- `page_checks` (Block Set) The set of checks to run against the currently loaded content. (see [below for nested schema](#nestedblock--step--page_checks))
...
<a id="nestedblock--step--actions"></a>
### Nested Schema for `step.actions`
...
- `click_action` (Block Set, Max: 1) The additional details needed for a CLICK, DOUBLE_CLICK or RIGHT_CLICK action type. (see [below for nested schema](#nestedblock--step--actions--click_action))
...
- `password_input_action` (Block Set, Max: 1) The additional details needed for a PASSWORD_INPUT action type. (see [below for nested schema](#nestedblock--step--actions--password_input_action))
- `scroll_to_element` (Block Set, Max: 1) The additional details needed for the SCROLL_TO_ELEMENT action type. (see [below for nested schema](#nestedblock--step--actions--scroll_to_element))
- `text_input_action` (Block Set, Max: 1) The additional details needed for a TEXT_INPUT action type. (see [below for nested schema](#nestedblock--step--actions--text_input_action))
...
<a id="nestedblock--step--actions--click_action"></a>
### Nested Schema for `step.actions.click_action`
...
<a id="nestedblock--step--actions--password_input_action"></a>
### Nested Schema for `step.actions.password_input_action`
...
<a id="nestedblock--step--actions--scroll_to_element"></a>
### Nested Schema for `step.actions.scroll_to_element`
...
<a id="nestedblock--step--actions--text_input_action"></a>
### Nested Schema for `step.actions.text_input_action`
...
<a id="nestedblock--step--page_checks"></a>
### Nested Schema for `step.page_checks`
...
- `check_console_log` (Block Set) Check for a log entry made after the past step. (see [below for nested schema](#nestedblock--step--page_checks--check_console_log))
- `check_current_url` (Block Set) Check the URL of the current page. (see [below for nested schema](#nestedblock--step--page_checks--check_current_url))
- `check_element_on_page` (Block Set) Check for a specific element and it's attributes on the current page. (see [below for nested schema](#nestedblock--step--page_checks--check_element_on_page))
- `check_for_text` (Block Set) Check a specific stirng is present or absent on the current page. (see [below for nested schema](#nestedblock--step--page_checks--check_for_text))
- `check_url_response` (Block Set) Check a network request made after the previous step. (see [below for nested schema](#nestedblock--step--page_checks--check_url_response))
...
<a id="nestedblock--step--page_checks--check_console_log"></a>
### Nested Schema for `step.page_checks.check_console_log`
...
<a id="nestedblock--step--page_checks--check_current_url"></a>
### Nested Schema for `step.page_checks.check_current_url`
...
<a id="nestedblock--step--page_checks--check_element_on_page"></a>
### Nested Schema for `step.page_checks.check_element_on_page`
...
<a id="nestedblock--step--page_checks--check_for_text"></a>
### Nested Schema for `step.page_checks.check_for_text`
...
<a id="nestedblock--step--page_checks--check_url_response"></a>
### Nested Schema for `step.page_checks.check_url_response`

Required:

- `alert_response_time` (Number) The response time in milliseconds that will trigger the check to fail.
- `comparison` (String) Must be one of EQUALS, DOES_NOT_EQUAL, STARTS_WITH, ENDS_WITH, CONTAINS or DOES_NOT_CONTAIN. The way to compare the given url against the network requets made.
